#-----------------------------------------------------------------------------#
#                    Spatial validation - DEML ensemble                      #
#-----------------------------------------------------------------------------#
# Description:
# This script trains and evaluates the spatial DEML ensemble for indoor
# temperature and humidity. Participants are split into spatial training and
# testing groups, base models are trained, and their predictions are combined
# with meta-models and NNLS weights.
#
# Outputs:
# - results/Model/Spacial_Validation_seed1225/Temperature/*
# - results/Model/Spacial_Validation_seed1225/Humidity/*
#-----------------------------------------------------------------------------#
...
path_out <- file.path(project_root, "results", "Model", "Spacial_Validation_seed1225", "")
...
source(file.path(project_root, "script", "indoor_temp", "DEML", "Spacial_Validation", "10_DEML_Function_spacial.R"))
...
set.seed(1225)  # Set a seed for reproducibility

test_ids <- temp_hr_model %>%
  distinct(id_mother) %>%
  sample_frac(0.2) %>%
  pull(id_mother)

testset <- temp_hr_model %>%
  filter(id_mother %in% test_ids)
...
write_csv(temp_importance_rf, paste0(path_out,"table_figure/Temperature/varimp_DEML_train_rf_spacial.csv"))
...
write_csv(hr_importance_rf, paste0(path_out,"table_figure/Humidity/varimp_DEML_train_rf_spacial.csv"))
...
write_csv(temp_importance_xgb, paste0(path_out,"table_figure/Temperature/varimp_DEML_train_xgb_spacial.csv"))
...
write_csv(hr_importance_xgb, paste0(path_out,"table_figure/Humidity/varimp_DEML_train_xgb_spacial.csv"))
...
write_csv(temp_importance_gbm, paste0(path_out,"table_figure/Temperature/varimp_DEML_train_gbm_spacial.csv"))
...
write_csv(hr_importance_gbm, paste0(path_out,"table_figure/Humidity/varimp_DEML_train_gbm_spacial.csv"))
...
temp_dummy_train_pred <-
  cbind(
    trainset_dummy_temp[, colnames(trainset_dummy_temp) %in% c("gid","date","week","id_week") == FALSE],
...
temp_train_pred <-
  cbind(
    trainset_temp[, colnames(trainset_temp) %in% c("gid","date","week","id_week") == FALSE],
    temp_rf_pred = temp_pred_train_rf,
    temp_xgb_pred = temp_pred_train_xgb,
    temp_gbm_pred = temp_pred_train_gbm
  )
...
temp_dummy_test_pred <-
  cbind(
    testset_dummy_temp[, colnames(testset_dummy_temp) %in% c("gid","date","week","id_week") == FALSE],
...
temp_test_pred <-
  cbind(
    testset_temp[, colnames(testset_temp) %in% c("gid","date","week","id_week") == FALSE],
    temp_rf_pred = temp_pred_rf,
    temp_xgb_pred = temp_pred_xgb,
    temp_gbm_pred = temp_pred_gbm
  )
...
hr_dummy_train_pred <-
  cbind(
    trainset_dummy_hr[, colnames(trainset_dummy_hr) %in% c("gid","date","week","id_week") == FALSE],
...
hr_train_pred <-
  cbind(
    trainset_hr[, colnames(trainset_hr) %in% c("gid","date","week","id_week") == FALSE],
    hr_rf_pred = hr_pred_train_rf,
    hr_xgb_pred = hr_pred_train_xgb,
    hr_gbm_pred = hr_pred_train_gbm
  )
...
hr_dummy_test_pred <-
  cbind(
    testset_dummy_hr[, colnames(testset_dummy_hr) %in% c("gid","date","week","id_week") == FALSE],
...
hr_test_pred <-
  cbind(
    testset_hr[, colnames(testset_hr) %in% c("gid","date","week","id_week") == FALSE],
    hr_rf_pred = hr_pred_rf,
    hr_xgb_pred = hr_pred_xgb,
    hr_gbm_pred = hr_pred_gbm
  )
...
#==================          temperature         ========================# 
temp_dummy_train_pred_glm <- temp_dummy_train_pred %>% select(-id_mother)
temp_dummy_test_pred_glm <- temp_dummy_test_pred %>% select(-id_mother)
...
#==================          humidity         ========================# 
hr_dummy_train_pred_glm <- hr_dummy_train_pred %>% select(-id_mother)
hr_dummy_test_pred_glm <- hr_dummy_test_pred %>% select(-id_mother)
...
# save performance
write_csv(temp_all_test, paste0(path_out,"Temperature/temp_perf_DEML_spacial.csv")) 
...
# save performance
write_csv(hr_all_test, paste0(path_out,"Humidity/hr_perf_DEML_spacial.csv")) 
