#-----------------------------------------------------------------------------#
#                    Short-term daily validation - DEML                        #
#-----------------------------------------------------------------------------#
# Description:
# This script runs the short-term daily DEML validation workflow. It loads the
# indoor model data, splits observations by participant-week, trains base models,
# combines their predictions with a meta-ensemble, and saves daily validation
# results for temperature and humidity.
#
# Outputs:
# - results/Model/Temporal_Validation_daily/Temperature/*.csv
# - results/Model/Temporal_Validation_daily/Humidity/*.csv
# - results/Model/Temporal_Validation_daily/*.rds
#-----------------------------------------------------------------------------#
...
path_out <- file.path(project_root, "results", "Model", "Temporal_Validation_daily")

if (!dir.exists(path_out)) dir.create(path_out, recursive = TRUE, showWarnings = FALSE)
...
source(file.path(project_root, "script", "indoor_temp", "DEML", "Temporal_shortterm_validation", "10_DEML_Function_daily.R"))
...
temp_hr_model <- temp_preidctors_data_model %>%
  select("gid", "id_week", "id_mother", "date","sr","apavg","wind_speed_10m",
         "wind_direction_10m","precipitation", "ndvi_300","maternal_age", "temperature_min_home",
         "temperature_mean_home", "temperature_max_home", "humidity_mean_home","hum_mean_lag1_avg",
         "temp_mean_lag1_avg", "difficult_pay_heating", "ct1","ct3_8a",
         "ct6_m01", "ct7_m01","ct8ec","ct8m","ct8l_f","ct8k","ct8l_c","ct8g",
         "ct8h","ct8i","ct8j","heating_system_bedroom", "heating_system_livingroom",
         "EC_2_32w","GS3_m01","r6_1","r6_2","r6_3","r6_4","r6_5","r6_6","h5","N6_m01",
         "N6_m03","n7_1a","n7_1b","n7_2a","n7_2b","n7_3a","n7_3b","cooling_system_day",
         "cooling_system_night","Average_building","VALOR_AILLAMENTS_300m",
         "VALOR_FINESTRES_300m","Energia_calefacció_demanda_300m","Energia_refrigeració_demanda_300m",
         "indoor_temp","indoor_hr","year","month","yday","week")
...
testset <- temp_hr_model %>%
  ...
  filter(n() >= 6) %>% 
  sample_n(1) %>%  # Randomly select 1 day per group
  ungroup()
...
trainset<- temp_hr_model %>%
  anti_join(testset, by=c("id_mother","date","week"))
...
write_csv(temp_importance_rf, paste0(path_out,"table_figure/Temperature/varimp_DEML_train_rf_random_day.csv"))
...
write_csv(hr_importance_rf, paste0(path_out,"table_figure/Humidity/varimp_DEML_train_rf_random_day.csv"))
...
write_csv(temp_importance_xgb, paste0(path_out,"table_figure/Temperature/varimp_DEML_train_xgb_random_day.csv"))
...
write_csv(hr_importance_xgb, paste0(path_out,"table_figure/Humidity/varimp_DEML_train_xgb_random_day.csv"))
...
write_csv(temp_importance_gbm, paste0(path_out,"table_figure/Temperature/varimp_DEML_train_gbm_random_day.csv"))
...
write_csv(hr_importance_gbm, paste0(path_out,"table_figure/Humidity/varimp_DEML_train_gbm_random_day.csv"))
...
temp_dummy_train_pred <-
  cbind(
    trainset_dummy_temp[, colnames(trainset_dummy_temp) %in% c("gid","date","id_week") == FALSE],
...
temp_train_pred <-
  cbind(
    trainset_temp[, colnames(trainset_temp) %in% c("gid","date","id_week") == FALSE],
    temp_rf_pred = temp_pred_train_rf,
    temp_xgb_pred = temp_pred_train_xgb,
    temp_gbm_pred = temp_pred_train_gbm
  )
...
temp_dummy_test_pred <-
  cbind(
    testset_dummy_temp[, colnames(testset_dummy_temp) %in% c("gid","date","id_week") == FALSE],
...
temp_test_pred <-
  cbind(
    testset_temp[, colnames(testset_temp) %in% c("gid","date","id_week") == FALSE],
    temp_rf_pred = temp_pred_rf,
    temp_xgb_pred = temp_pred_xgb,
    temp_gbm_pred = temp_pred_gbm
  )
...
hr_dummy_train_pred <-
  cbind(
    trainset_dummy_hr[, colnames(trainset_dummy_hr) %in% c("gid","date","id_week") == FALSE],
...
hr_train_pred <-
  cbind(
    trainset_hr[, colnames(trainset_hr) %in% c("gid","date","id_week") == FALSE],
    hr_rf_pred = hr_pred_train_rf,
    hr_xgb_pred = hr_pred_train_xgb,
    hr_gbm_pred = hr_pred_train_gbm
  )
...
hr_dummy_test_pred <-
  cbind(
    testset_dummy_hr[, colnames(testset_dummy_hr) %in% c("gid","date","id_week") == FALSE],
...
hr_test_pred <-
  cbind(
    testset_hr[, colnames(testset_hr) %in% c("gid","date","id_week") == FALSE],
    hr_rf_pred = hr_pred_rf,
    hr_xgb_pred = hr_pred_xgb,
    hr_gbm_pred = hr_pred_gbm
  )
...
#==================          temperature         ========================# 
temp_dummy_train_pred_glm <- temp_dummy_train_pred %>% select(-id_mother, -week)
temp_dummy_test_pred_glm <- temp_dummy_test_pred %>% select(-id_mother, -week)
...
#==================          humidity         ========================# 
hr_dummy_train_pred_glm <- hr_dummy_train_pred %>% select(-id_mother, -week)
hr_dummy_test_pred_glm <- hr_dummy_test_pred %>% select(-id_mother, -week)
set.seed(1234)
hr_glm_meta <- glm(
  indoor_hr ~ .,
  family = gaussian,
...
# save performance
write_csv(temp_all_test, paste0(path_out,"Temperature/temp_perf_DEML_random_day.csv")) 
...
# save performance
write_csv(hr_all_test, paste0(path_out,"Humidity/hr_perf_DEML_random_day.csv")) 
